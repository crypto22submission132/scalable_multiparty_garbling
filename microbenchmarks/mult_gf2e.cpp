...
bpo::options_description programOptions() {
  bpo::options_description desc("Benchmark multiplication over GF2E.");
  desc.add_options()
    ("field_degree,d", bpo::value<uint32_t>()->default_value(8), "Degree of the polynomial modulus of the extension field.")
    ("num,n", bpo::value<uint32_t>()->default_value(1000000), "Number of multiplications.")
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (opts.count("output") != 0) {
      ...
      fs::path filename =
          (boost::format("multgf2e_d%1%_t%2%.json") % field_degree % threads)
              .str();
    ...
    for (size_t r = 0; r < repeat; r++) {
      ...
      auto ctime_mult = ctime / num;
      std::cout << "Repetition " << (r + 1) << ":\t" << ctime << " ms\t"
                << ctime_mult << " ms/multiplication" << std::endl;
    }

    std::cout << "\nAverage time:\t" << total_time / repeat << " ms\t"
              << total_time / (repeat * num) << " ms/multiplication"
              << std::endl;
